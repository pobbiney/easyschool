...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (DB::table('user_links')->where('link_url', 'promotion-settings')->exists()) {
            return;
        }

        $linkId = DB::table('user_links')->insertGetId([
            'link_url' => 'promotion-settings',
            'link_name' => 'Promotion Settings',
            'link_target' => null,
            'link_image' => null,
            'link_parent' => $settingsParent->link_id,
            'page_id' => 'settings',
            'page_id_sub' => 'promotion-settings',
            'status' => 'Active',
        ]);
    ...
    {
        $link = DB::table('user_links')->where('link_url', 'promotion-settings')->first();
