<div class="edit-sidebar bg-white position-fixed end-0 top-0 h-100vh overflow-y-auto z-99 max-w-700-px w-100 translate-x-full duration-300 active-translate-0">
    <div class="px-20 py-12 border-bottom d-flex align-items-center justify-content-between gap-20">
        <h5 class="text-lg mb-0">Edit Academic Term</h5>
        <button type="button" class="close-edit-sidebar text-danger-600 text-lg d-flex">
            <i class="ri-close-large-line"></i>
        </button>
    </div>
    <form method="POST" action="{{ route('update-academic-term-process') }}" class="d-flex flex-column p-20">
        @csrf
        <input type="hidden" name="term_id" id="edit_term_id">
        <div class="row g-3">
            <div class="col-sm-12">
                <label class="text-sm fw-semibold text-primary-light d-inline-block mb-8">Term Name</label>
                <input type="text" class="form-control" name="name" id="edit_term_name" placeholder="e.g. First Term">
                @error('name') <small class="text-danger">{{ $message }}</small> @enderror
            </div>
            <div class="col-sm-12">
                <label class="text-sm fw-semibold text-primary-light d-inline-block mb-8">Sort Order</label>
                <input type="number" class="form-control" name="sort_order" id="edit_term_sort_order" min="1" max="99">
            ...
            <div class="col-sm-12">
                ...
                <select class="form-control form-select" name="status" id="edit_term_status">
